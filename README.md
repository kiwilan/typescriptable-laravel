...
-   `embed` option (default to `false`): generate some global components methods, you need to install `typescriptable-laravel`, `ziggy-js` package and `InertiaTyped` Vue plugin from `typescriptable-laravel`.
    -   `$route`
    -   `$isRoute`
    -   `$currentRoute`
...
> -   [typescriptable-laravel](https://www.npmjs.com/package/typescriptable-laravel) for `npm` (optional for `embed` option, `kiwilan/typescriptable-laravel` package is not required): `npm install typescriptable-laravel --save-dev`
